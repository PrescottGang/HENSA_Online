...
// Axios instance avec token JWT automatique
...
const emptyForm = {
  ...
  matricule: "", classe_id: "", niveau: "", specialite: "", grade: "",
};

// ⚠️ IMPORTANT: Assurez-vous qu'il n'y a qu'UNE SEULE déclaration de ce composant
const UsersManagement = () => {
  ...
  const fetchStudents = async () => {
    ...
    try { 
      ...
      console.log("Étudiants chargés:", res.data.length);
    } catch (err) { 
      console.error("Erreur chargement étudiants:", err.response?.data || err.message);
  ...
  const fetchTeachers = async () => {
    ...
    try { 
      ...
      console.log("Enseignants chargés:", res.data.length);
    } catch (err) { 
      console.error("Erreur chargement enseignants:", err.response?.data || err.message);
  ...
  const fetchClasses = async () => {
    ...
    try { 
      ...
      console.log("Classes reçues:", res.data);
      setClasses(res.data); 
      
      if (res.data.length === 0) {
        showNotification("Aucune classe trouvée dans la base de données", "warning");
      }
    } catch (err) { 
      console.error("Erreur chargement classes:", err.response?.data || err.message);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formType === "student") {
        if (!form.matricule || !form.classe_id) {
          setError("Tous les champs sont requis pour un étudiant");
          setLoading(false);
          return;
        }

        const response = await apiClient.post(`/users/etudiants`, {
          nom: form.nom.trim(), 
        ...
        await fetchStudents();
        showNotification(response.data.message || "Étudiant ajouté avec succès");
      } else {
        const response = await apiClient.post(`/users/enseignants`, {
          nom: form.nom.trim(), 
          prenom: form.prenom.trim(), 
          email: form.email.trim().toLowerCase(),
          mot_de_passe: form.password, 
        ...
        await fetchTeachers();
        showNotification(response.data.message || "Enseignant ajouté avec succès");
      }
  ...
  const filteredStudents = students.filter((s) =>
    [s.nom, s.prenom, s.email, s.matricule, s.niveau, s.classe_nom].some((v) =>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        {/* Notification */}
        {notification.show && (
          <div className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-lg shadow-lg text-sm font-medium animate-slideIn flex items-center gap-2 ${
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {students.length} étudiants · {teachers.length} enseignants
                </p>
              </div>
            </div>
              ...
              {loadingData.students ? (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-gray-500 uppercase text-xs">
                    <tr>
                      {["Étudiant", "Email", "Matricule", "Niveau", "Classe", "Statut", "Actions"].map((h) => (
                    ...
                    {filteredStudents.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="text-center py-20 text-gray-400">
                          {searchQuery ? "Aucun résultat trouvé" : "Aucun étudiant trouvé"}
                        </td>
                      </tr>
            ...
            <div className="overflow-x-auto">
              {loadingData.teachers ? (
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 text-gray-500 uppercase text-xs">
                    <tr>
                      {["Enseignant", "Email", "Spécialité", "Grade", "Statut", "Actions"].map((h) => (
                        <th key={h} className="px-4 py-3 text-left font-medium">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {filteredTeachers.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="text-center py-20 text-gray-400">
                          {searchQuery ? "Aucun résultat trouvé" : "Aucun enseignant trouvé"}
                        </td>
                      </tr>
                      ...
                      <tr key={t.id} className="hover:bg-gray-50 transition">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div className="h-8 w-8 rounded-full bg-purple-100 text-purple-700 flex items-center justify-center text-xs font-semibold flex-shrink-0">
              ...
              <div className="p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {formType === "student" ? (
                    <>
                      <input 
                        placeholder="Matricule (ex: ETU2024001) *" 
                      ...
                      <input 
                        placeholder="Niveau (ex: L1, L2, M1...)" 
                        value={form.niveau}
                        required
                      ...
                      {classes.length === 0 && !loadingData.classes && (
                        <p className="text-xs text-amber-600 mt-1">
                          ⚠️ Aucune classe trouvée. Veuillez d'abord créer des classes.
                        </p>
                      )}
                    </>
                  ) : (
                    <>
                      <input 
                        placeholder="Spécialité (ex: Informatique)" 
                        value={form.specialite}
                        onChange={(e) => setForm({ ...form, specialite: e.target.value })} 
                        className={inputClass} 
                      />
                      <input 
                        placeholder="Grade (ex: Maître de conférences)" 
                        value={form.grade}
