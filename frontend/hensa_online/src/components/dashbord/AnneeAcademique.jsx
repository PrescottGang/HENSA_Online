...
import {
  Plus, X, Trash2, LockKeyhole, BookOpen,
  AlertCircle, CheckCircle, Search, Wand2,
} from "lucide-react";
...
const inputClass = "mt-1 w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500";
...
const AcademicYears = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await apiClient.post("/annees", form);
      showNotification("Année académique créée avec succès");
      setForm(emptyForm);
      setOpen(false);
      fetchYears();
    } catch (err) {
  ...
  const handleGenerateClasses = async (id) => {
    if (!window.confirm("Voulez-vous générer les classes ? Cette action est unique par année.")) return;
    setLoading(true);
    try {
  ...
  const handleAction = async () => {
    ...
    try {
      if (action === "cloturer") await apiClient.patch(`/annees/${id}/cloturer`);
  ...
  const filtered = years.filter((y) => {
    const matchesSearch = y.libelle.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesTab = activeTab === "ACTIVE" ? y.statut === "ACTIVE" : y.statut !== "ACTIVE";
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="flex border-b border-gray-200 px-4">
            {["ACTIVE", "CLOTUREE"].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition ${
                  activeTab === tab ? "border-blue-700 text-blue-700" : "border-transparent text-gray-500"
                }`}>
                {tab === "ACTIVE" ? "En cours" : "Archives"}
              </button>
            ))}
          </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.length === 0 ? (
                  ...
                  <tr key={y.id} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4 font-semibold text-gray-800">{y.libelle}</td>
                    <td className="px-6 py-4 text-gray-600">{formatDate(y.date_debut)}</td>
                    <td className="px-6 py-4 text-gray-600">{formatDate(y.date_fin)}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        y.statut === "ACTIVE" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"
                      }`}>
                        {y.statut}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        {y.statut === "ACTIVE" && (
                          <button onClick={() => handleGenerateClasses(y.id)}
                            className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition"
                            title="Générer les classes">
                            <Wand2 className="h-4 w-4" />
                          </button>
                        )}
                        <button onClick={() => setConfirmDialog({ open: true, id: y.id, action: "cloturer" })}
                          className="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition"
                          title="Clôturer">
                          <LockKeyhole className="h-4 w-4" />
                        </button>
                        <button onClick={() => setConfirmDialog({ open: true, id: y.id, action: "delete" })}
      ...
      {confirmDialog.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
            <h3 className="font-semibold text-gray-800 mb-2">Confirmer l'action</h3>
            <p className="text-sm text-gray-500 mb-6">
              Êtes-vous sûr de vouloir continuer ? Cette action peut être irréversible.
            </p>
            <div className="flex gap-3">
              ...
              <button onClick={handleAction}
                className={`flex-1 py-2 rounded-lg text-white text-sm font-medium ${
                  confirmDialog.action === "delete" ? "bg-red-600" : "bg-amber-600"
                }`}>
                Confirmer
              </button>
            </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            ...
            {/* ✅ onSubmit correctement relié à handleSubmit */}
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase">Libellé</label>
                <input required className={inputClass} value={form.libelle}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" required className={inputClass} value={form.date_debut}
                    onChange={(e) => setForm({ ...form, date_debut: e.target.value })} />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">Fin</label>
                  <input type="date" required className={inputClass} value={form.date_fin}
